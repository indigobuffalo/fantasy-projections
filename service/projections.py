...
class ProjectionsSvc:
    def __init__(self, readers: list[FantasyBaseDao]):
    ...
    @staticmethod
    def get_matches(reader: FantasyBaseDao, regexes: list[str] = ['.*']) -> DataFrame:
    ...
    @staticmethod
    def _filter_excluded_players(reader: FantasyBaseDao, results: DataFrame, excluded: list[str]) -> DataFrame:
        if len(excluded) == 0:
            return results
    ...
    @staticmethod
    def _filter_positions(reader: FantasyBaseDao, results: DataFrame, positions_rgx) -> DataFrame:
    ...
    def refine_results(self, reader: FantasyBaseDao, results: DataFrame, positions_rgx: list[str], excluded: list[str] = None, count: int = -1) -> DataFrame:
    ...
    @staticmethod
    def register_to_averaged_rankings(reader: FantasyBaseDao, results: DataFrame, average_rankings: dict):
        for player_name in results[reader.primary_col]:
            reader.add_to_averaged_rankings(player_name, average_rankings)

    def get_matches_for_readers(self, regexes: list[str]) -> dict[FantasyBaseDao, DataFrame]:
        '''
    ...
    def print_matches_for_all_readers(self, reader_results_map: dict[FantasyBaseDao, DataFrame]) -> None:
